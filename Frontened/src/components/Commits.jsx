...
const Commits = ({ repoName, repoOwner, branch }) => {
  const [commits, setCommits] = useState([]);

  useEffect(() => {
    if (!repoName || !repoOwner || !branch) return;
  ...
  }, [repoName, repoOwner, branch]); // ✅ FIXED dependencies
    ...
    <div className="rounded-2xl p-8 bg-white/60 backdrop-blur-xl border border-slate-200 shadow-xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="ml-auto text-sm text-slate-600 bg-slate-100 px-3 py-1 rounded-full">
          {commits.length ? `${commits.length} commits` : "No commits"}
        </span>
      </div>
      ...
      {commits.length > 0 ? (
        <div className="space-y-4">
          {commits.map((commit, index) => (
            <div
              key={commit.sha}
              className="p-5 rounded-xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition flex items-start gap-4 relative"
            >
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 text-xs text-slate-500">
                  ...
                  <code className="px-2 py-1 bg-slate-200 rounded font-mono text-xs">
